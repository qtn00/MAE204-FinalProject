...
[traj1,gripper_state1] = ScrewTrajectory_modified(T_se,T_se_stand_ini,N ,dt,'open',3);
% Moving to grasping position at inital cube position and grap the cube
[traj2,gripper_state2] = ScrewTrajectory_modified(T_se_stand_ini,T_se_g_ini,N ,dt,'open',3);
% Moving back to standoff
[traj3,gripper_state3] = ScrewTrajectory_modified(T_se_g_ini,T_se_stand_ini,N,dt,'close',3);
% Moving to final standoff position
[traj4,gripper_state4] = ScrewTrajectory_modified(T_se_stand_ini,T_se_stand_fi,N,dt,'close',3);
% Moving to final cube position and release the cube
[traj5,gripper_state5] = ScrewTrajectory_modified(T_se_stand_fi,T_se_g_fi,N,dt,'close',3);
% Moving back to final standoff
[traj6,gripper_state6] = ScrewTrajectory_modified(T_se_g_fi,T_se_stand_fi,N,dt,'open',3);
